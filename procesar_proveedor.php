...
$verificarUsuario = "SELECT NIT FROM proveedor WHERE NIT = '$identificacion'";
...
if ($resultado->num_rows > 0) {
    ...
    $sqlClientes = "INSERT INTO proveedor (NIT, Nombre,Nombre_Contacto,Direccion) VALUES ('$identificacion', '$nombre','$contacto','$direccion')";

    if ($conn->query($sqlClientes) === TRUE) {
        // Insertar datos en la tabla telefono_cliente solo si el teléfono 2 no está vacío
        if (!empty($telefono2)) {
            $sqlTelefono = "INSERT INTO telefono_proveedor (NIT, Telefono) VALUES ('$identificacion', '$telefono1'), ('$identificacion', '$telefono2')";
        } else {
            $sqlTelefono = "INSERT INTO telefono_proveedor (NIT, Telefono) VALUES ('$identificacion', '$telefono1')";
        }
